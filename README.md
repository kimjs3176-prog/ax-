...
| 기능 | 내용 |
|---|---|
| 공유 링크 | 주소로 화면 공유: `#/meetings/회의ID`, `#/issues/쟁점ID`, `#/brief` |
| 회기 선택 | 화면 상단에서 회기(예: 제418회 정기회)를 고르면 회의록·검색·쟁점 상세·브리핑이 그 회기로 좁혀짐 |
| 대시보드 | 첫 화면 요약(최근 회기·가장 많이 다룬 쟁점·핵심 수치), **회기별 쟁점 흐름 히트맵**, **3D 위원–쟁점–기관 신경망**, 쟁점 순위, 최근 회의, 최근 정부 약속 |
| 회의록 | **회의록 중심 화면**: 상단 통합 검색(발언자 필터·회기별 언급·관련 쟁점), 회의 목록, 회의 상세 탭(요약 / 전체 회의록(안건 구분·발언자 필터·회의 내 찾기) / 질의응답 / 약속·자료요구 / 안건), **내 AI로 분석**(ChatGPT·Claude 새 창 열기 + 전문 클립보드 복사, 전문 Markdown 링크·다운로드 `/api/meetings/{id}/export.md`) |
| 의원 분석 | 의원별 관심 쟁점·질의 관점(예산·감독·피해 등, 전체 의원 평균 대비)·핵심어(TF-IDF, 명사만)·상대 기관·회기별 질의, 과거 질의·받아낸 약속·반복 제기 핵심어 기반 **예상 질문**. 범위: 전체/회기/최근 회의 5·10건, 회의 상세의 「의원별 관점」 탭(`/api/members`, `/api/members/{이름}?meetings=`) |
| 쟁점 분석 | 쟁점 분류(SKOS 상·하위) → 쟁점 상세: 회기별 추이, 관련 기관·질의 위원, 질의(왼쪽)–정부 답변·약속(오른쪽) 대화 |
...
| 고급 도구(하단 링크) | SPARQL 지식그래프 직접 질의(예시 질의 제공), 전체 그래프 TTL 내려받기 |
